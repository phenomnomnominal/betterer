...
import type { SourceFile, ModuleKind } from 'typescript';
...
export async function run(logger: BettererLogger, configPath: string, save: boolean): Promise<void> {
  ...
  const upgrade = moduleType === ts.ModuleKind.CommonJS ? upgradeCJS : upgradeESM;
  ...
  const formattedOriginal = prettier.format(fileText, formatOptions);
  const formatted = prettier.format(printed, formatOptions);
