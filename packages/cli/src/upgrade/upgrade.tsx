        ...
        {configPaths.map((configPath) => {
          const runUpgradeConfigFile = useCallback(
            async (logger: BettererLogger) => {
              const upgradeConfigFile: UpgradeConfigFileWorker = importWorker('./upgrade-config-file.worker.js');
              try {
                await upgradeConfigFile.api.run(exposeMain(logger), path.resolve(cwd, configPath), save);
              } finally {
