...
export function codeΔ(codeInfo: BettererLoggerCodeInfo): string {
  const line = isNaN(codeInfo.line) ? 0 : codeInfo.line;
  const column = isNaN(codeInfo.column) ? 0 : codeInfo.column;
  ...
  const startLocation = { line, column };
