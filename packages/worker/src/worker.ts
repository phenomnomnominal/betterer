...
export function importWorker<T>(requirePath: string): BettererWorkerAPI<T> {
  ...
  const idPath = path.resolve(path.dirname(callerFilePath), requirePath);
...
export function exposeMain<Expose>(api: Expose): Expose {
  ...
  Object.keys(api).forEach((k) => {
    const key = k as keyof Expose;
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment -- `proxy` has bad types using `{}` 😭.
    // @ts-ignore
    api[key] = exposeMain<Expose[typeof key]>(api[key]);
  });
  return api;
}
