...
describe('betterer', () => {
  it('should report a worse file coverage result', async () => {
    ...
    const fixture = await createFixture('coverage-files-worse', {
      '.betterer.js': `
      ...
      'tsconfig.json': `
{
  ...
  "${file2Path}": {
    ...
    await fixture.writeFile(coveragePath, stillWorseCoverage);

    const stillWorseTestRun = await betterer({ configPaths, resultsPath, workers: false });

    expect(fixture.testNames(stillWorseTestRun.worse)).toEqual(['test']);
    ...
    await fixture.writeFile(coveragePath, evenWorseCoverage);

    const evenWorseTestRun = await betterer({ configPaths, resultsPath, workers: false });

    expect(fixture.testNames(evenWorseTestRun.worse)).toEqual(['test']);
